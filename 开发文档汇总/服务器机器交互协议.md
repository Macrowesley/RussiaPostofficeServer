...
目的：记录批次的开始结束金额，数量，累加所有批次，然后得到一次打印中的总金额和数量，防止断电情况



关键信息：总数量，总金额，重量

一次打印任务中，机器发送什么数据给服务器？
...
开始打印任务：总金额  总件数  二维码信息（空的） **然后服务器返回一个TransactionID**

一批结束：总金额+  总件数+  二维码信息（实际一批总金额/单张邮票金额、数量、重量）

新批次开始：总金额  总件数  二维码信息（有效数据）

结束：总金额  总件数  二维码信息（有效数据）
...
如果断电：机器发我的金额和服务器的不一致

机器的金额比数据库的大，更新数据库信息：总信息和订单信息

差值的金额和数量，最后一批，开始时候发送的二维码内容，就能知道具体多少张二维码
...
（机器发送结果给服务器：是否收到privateKey，然后服务器发送公钥给俄罗斯）
...
typedef  struct{
    ...
    unsigned char acnum[6];             //机器表头号
    unsigned char version[3];           //版本号
    unsigned char content[?];			//加密后内容: PublicKeyFMDTO信息 + frankMachineId
    unsigned char check;				//校验位
    unsigned char tail;					//0xD0
...
typedef  struct{
    ...
    unsigned char acnum[6];             //机器表头号
    unsigned char version[3];           //版本号
    unsigned char content[?];			//加密后内容: result(1成功 0 失败)
    unsigned char check;				//校验位
    unsigned char tail;					//0xD0
}__attribute__((packed))privateKeyRes, *privateKeyRes;
```
